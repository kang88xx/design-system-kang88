"""Local static catalog server with byte ranges for reference-video seeking."""
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--port',type=int,default=40565);args=parser.parse_args()
    server=http.server.ThreadingHTTPServer(('127.0.0.1',args.port),functools.partial(Handler,directory=str(ROOT)))
    print(f'Family design system: http://127.0.0.1:{server.server_port}',flush=True)
