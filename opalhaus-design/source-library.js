(function () {
  ...
  const TEXT = new Set([".css", ".mjs", ".js", ".json", ".html", ".md", ".txt", ".svg", ".py", ".cjs"]);
  const labels = {
  ...
  async function loadData() {
    if (window.OpalSourceLibrary && Array.isArray(window.OpalSourceLibrary.items)) return window.OpalSourceLibrary;
  ...
  function renderStatus(root) {
    ...
    setText(copy, `공개 페이지·배포 모듈·폰트·영상·이미지와 의미별 추출 규칙·재사용 코드를 포함했습니다. 직접 내려받지 못한 리소스는 ${unavailable}개, 편집 원본·상태 검증 등 범위를 구분한 항목은 ${gaps.length}개입니다.`);
    ...
    recon.href = "layout-recipes.html";
    const strong = document.createElement("strong");
    setText(strong, "컴포넌트와 레이아웃 적용 예제");
  ...
  function preview(item, large) {
    ...
    if (path && IMAGE.has(type)) {
      ...
      const family = `OpalSourceFont${Math.abs(hash(path))}`;
      ...
      sample.style.fontFamily = `${family}, "Inter Tight", Arial, sans-serif`;
      setText(sample, "Opalhaus Aa");
      wrap.append(sample);
    } else {
  ...
  async function inspectItem(item) {
    let dialog = $("#source-inspector");
    if (!dialog) { dialog = document.createElement("dialog"); dialog.id="source-inspector"; dialog.className="source-dialog"; dialog.setAttribute("aria-label","소스 상세 검사"); document.body.append(dialog); }
    let fullText = "";
  ...
  async function init() {
    ...
    renderRoot(root);
    renderRuleLibrary(root);
  }

  function renderRuleLibrary(root) {
    const specs=state.data.componentSpecs||[];
    if(!specs.length)return;
    const panel=document.createElement("div"); panel.className="rule-library"; panel.id="rule-library";
    panel.innerHTML='<div class="rule-title"><span>EXTRACTED COMPONENT RULES</span><h3>컴포넌트의 실제 CSS를 꺼내세요.</h3><p>공개 CSS 선언과 breakpoint를 그대로 보여줍니다. 전체 번들에서 필요한 셀렉터를 찾는 출발점입니다.</p></div><div class="rule-controls"><label>컴포넌트 <select data-rule-component></select></label><label>속성 / 셀렉터 검색 <input data-rule-search type="search" placeholder="padding, radius, font-size…"></label></div><p data-rule-state></p><div data-rule-results></div>';
    root.append(panel);
    const select=panel.querySelector('[data-rule-component]'); specs.forEach(c=>option(select,c.id,`${c.name} · ${c.rules.length} rules`));
    const search=panel.querySelector('[data-rule-search]');
    const render=()=>{
      const c=specs.find(c=>c.id===select.value)||specs[0];
      const result=panel.querySelector('[data-rule-results]'); result.replaceChildren();
      panel.querySelector('[data-rule-state]').textContent=`${c.sourceHtml} · ${c.rules.length} 원본 규칙 · ${Object.keys(c.states||{}).length} 캡처 상태 그룹`;
      const q=search.value.trim().toLowerCase(); const rules=c.rules.filter(r=>JSON.stringify(r).toLowerCase().includes(q));
      if(!rules.length){result.textContent='일치하는 규칙이 없습니다.';return;}
      rules.forEach((r,i)=>{
        const d=document.createElement('details'); d.className='rule-item'; d.open=i===0;
        const summary=document.createElement('summary'); summary.textContent=r.selector;
        const meta=document.createElement('p');meta.textContent=(r.conditions||[]).join(' / ')||'기본 규칙';
        const code=r.selector+' {\n'+Object.entries(r.declarations).map(([k,v])=>'  '+k+': '+v+';').join('\n')+'\n}';
        ...
        const button=document.createElement('button');button.type='button';button.textContent='CSS 복사';button.onclick=()=>copy(code);
        const link=document.createElement('a');link.href=r.source.file;link.target='_blank';link.rel='noreferrer';link.textContent=`원본 CSS ↗ · offset ${r.source.characterOffset}`;
        actions.append(button,link);d.append(summary,meta,pre,actions);result.append(d);
      });
    };
    select.addEventListener('change',render);search.addEventListener('input',render);render();
  }

  window.OpalSourceBrowser = { init, filtered, inspectItem };
