...
function getTablist(tabsRoot) {
  ...
  return queryAll(tabsRoot, "[role='tablist']").find((tablist) => belongsToScope(tablist, tabsRoot, "[data-tcs-tabs]")) || tabsRoot;
}
...
function getTabs(root, tabsRoot) {
  ...
  return queryAll(tablist, "[role='tab'], button").filter((tab, index, tabs) => {
    return tabs.indexOf(tab) === index && ownsTarget(root, tab) && belongsToScope(tab, tabsRoot, "[data-tcs-tabs]");
  });
}

function getAccordionItems(root, accordion) {
  return queryAll(accordion, "details").filter((item) => {
    return ownsTarget(root, item) && item.closest("[data-tcs-accordion]") === accordion;
  });
}

function getPanels(root, tabsRoot, tabs) {
  const fallbackPanels = queryAll(tabsRoot, "[role='tabpanel']").filter((panel) => {
    return ownsTarget(root, panel) && belongsToScope(panel, tabsRoot, "[data-tcs-tabs]");
  });
  return tabs.map((tab, index) => {
    const controlled = getById(root, tab.getAttribute("aria-controls"));
    if (controlled && tabsRoot.contains(controlled) && belongsToScope(controlled, tabsRoot, "[data-tcs-tabs]")) return controlled;
...
function activateTab(root, tabsRoot, tab, emit = true) {
  ...
  if (emit && panel) {
    dispatch(tabsRoot, "tcs:tabchange", { tabId: tab.id, panelId: panel.id });
  }
}

function setupTabs(root, remember, nextId) {
  queryAll(root, "[data-tcs-tabs]").forEach((tabsRoot) => {
...
function setupAccordions(root) {
  queryAll(root, "[data-tcs-accordion][data-exclusive='true']").forEach((accordion) => {
...
function setupDropdowns(root, remember) {
  queryAll(root, "[data-tcs-dropdown]").forEach((dropdown) => {
    if (!ownsTarget(root, dropdown)) return;
    const trigger = dropdown.querySelector("[data-tcs-dropdown-trigger]");
...
function createController(root) {
  ...
  const nextId = () => {
    generatedId += 1;
    return `tcs-tabs-${generatedId}`;
  };

  // Scroll-spy: [data-tcs-scrollspy] containers mark the link whose #target section is in view with aria-current="true".
  const observers = [];
  if (typeof IntersectionObserver !== "undefined") {
    queryAll(root, "[data-tcs-scrollspy]").forEach((spy) => {
      if (!ownsTarget(root, spy)) return;
      const links = Array.from(spy.querySelectorAll("a[href^='#']"));
      const pairs = links.map((link) => [link, getById(doc, link.getAttribute("href").slice(1))]).filter(([, target]) => target);
      if (!pairs.length) return;
      links.forEach((link) => remember(link, "aria-current", link.getAttribute("aria-current")));
      const margin = spy.getAttribute("data-tcs-scrollspy") || "-40% 0px -55% 0px";
      const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) return;
          pairs.forEach(([link, target]) => {
            if (target === entry.target) link.setAttribute("aria-current", "true");
            else link.removeAttribute("aria-current");
          });
          dispatch(spy, "tcs:sectionchange", { id: entry.target.id });
        });
      }, { rootMargin: margin });
  ...
  on(root, "click", (event) => {
    ...
    if (tab) {
      const tabsRoot = tab.closest("[data-tcs-tabs]");
      if (tabsRoot && ownsTarget(root, tabsRoot)) activateTab(root, tabsRoot, tab);
      return;
    }

    const opener = closestElement(event.target, "[data-tcs-dialog-open]", root);
    if (opener) {
      const id = opener.getAttribute("data-tcs-dialog-open");
      ...
      if (dialog instanceof HTMLDialogElement && ownsTarget(root, dialog) && !dialog.open) {
        ...
        dispatch(dialog, "tcs:dialogchange", { id, open: true });
      }
      return;
    }

    const closer = closestElement(event.target, "[data-tcs-dialog-close]", root);
    if (closer) {
    ...
    const trigger = closestElement(event.target, "[data-tcs-dropdown-trigger]", root);
    if (trigger) {
      const dropdown = trigger.closest("[data-tcs-dropdown]");
  ...
  on(root, "keydown", (event) => {
    ...
    const dropdown = closestElement(event.target, "[data-tcs-dropdown]", root);
  ...
  on(root, "toggle", (event) => {
    ...
    const accordion = details.closest("[data-tcs-accordion][data-exclusive='true']");
    if (!accordion || !ownsTarget(root, accordion)) return;
  ...
  queryAll(root, "dialog").forEach((dialog) => {
    on(dialog, "close", () => {
      ...
      dispatch(dialog, "tcs:dialogchange", { id, open: false });
...
export function initThreeCirclesSystem(root = defaultRoot()) {
  if (!root || typeof root.querySelectorAll !== "function") {
    throw new TypeError("initThreeCirclesSystem requires an HTMLElement or Document root.");
  }
