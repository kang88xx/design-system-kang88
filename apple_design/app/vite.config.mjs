...
function serveResearchDownloads(directory) {
  return async (req, res, next) => {
    ...
    if (!["/extraction.json", "/research/manifest.json", "/research/runtime-motion.json", "/research/motion-catalog.json", "/research-source-kit.tar.gz", "/source-design-system-0.1.0.tgz", "/motion-code-kit.tar.gz"].includes(pathname) || !["GET", "HEAD"].includes(req.method)) return next();
    try {
