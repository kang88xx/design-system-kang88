...
const capturePages=new Map([{id:'homepage',url:extraction.source||'https://www.apple.com/',rawDom:'/apple_design/app/dist/client/research/pages/homepage/raw.html'},...manifest.pages].map(page=>[page.id,page]));
...
manifest.runtimeMotion={file:'/apple_design/app/dist/client/research/runtime-motion.json',catalogFile:'/apple_design/app/dist/client/research/motion-catalog.json',pages:runtime.pages?.length||0,records:runtime.records?.length||0};
...
const derived=new Set(['/apple_design/app/dist/client/research/manifest.json','/apple_design/app/dist/client/research/completeness.json','/apple_design/app/dist/client/research/source-index.json']);
for(const file of await filesIn(pub)){
 ...
 if(local.startsWith('/apple_design/app/dist/client/source-local/')||['/apple_design/app/dist/client/reference-adapter.js','/apple_design/app/dist/client/local-api.js','/apple_design/app/dist/client/homepage.html'].includes(local)||local.startsWith('/apple_design/app/dist/client/research/observed-')||local.startsWith('/apple_design/app/dist/client/interaction-lab/')||/\/research\/(?:css-|dom-|dependency-|substitutions|runtime-motion|runtime-radio|runtime-dom|motion-catalog|motion-replay)/.test(local))provenance='local';
...
for(const item of unresolved){
 ...
 if(/\.(woff2?|ttf|otf)$/.test(pathname)){
  replacementFile='/apple_design/app/dist/client/fonts/source-fonts.css';
 ...
 }else if(pathname.includes('/apple_design/app/dist/client/assets/ac-buystrip/')){
  const candidate='/apple_design/app/dist/client/ac/globalfooter/8/en_US/assets/ac-buystrip/'+pathname.split('/apple_design/app/dist/client/assets/ac-buystrip/')[1];
  ...
  replacementFile='/apple_design/app/dist/client/research/replacements/productred.svg';replacementReason='Local red color-swatch substitute; original SVG unavailable.';
...
for(const local of ['/apple_design/app/dist/client/research/resource-substitutions.json',...new Set(resourceSubstitutions.map(r=>r.replacementFile))]){
