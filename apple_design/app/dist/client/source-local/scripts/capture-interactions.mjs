...
const browser=await chromium.launch({executablePath:'/apple_design/app/dist/client/home/kang/.cache/ms-playwright/chromium-1223/chrome-linux64/chrome',headless:true});
...
for(const group of groups){
 ...
 for(const [name,selector,clickSelector] of steps){
  ...
  try{
   ...
   entry.beforeScreenshot='/apple_design/app/dist/client/interaction-lab/'+id+'-before.png';entry.afterScreenshot='/apple_design/app/dist/client/interaction-lab/'+id+'-after.png';
   await fs.writeFile(root+'/'+id+'.json',JSON.stringify(entry,null,2));entry.file='/apple_design/app/dist/client/interaction-lab/'+id+'.json';
 ...
 if(group.id!=='mac'&&!retryOnly){
  ...
  const file='/apple_design/app/dist/client/interaction-lab/'+group.id+'-scroll.json';await fs.writeFile('app/public'+file,JSON.stringify(samples,null,2));scrollStudies.push({id:group.id,url:group.url,file,samples:samples.length});
 }
