...
const catalog=JSON.parse(await fs.readFile(pub+'/apple_design/app/dist/client/research/motion-catalog.json','utf8'));
const records=[];
...
const file='/apple_design/app/dist/client/research/motion-replay.json';
...
const files=['docs/motion-research.md','docs/motion-usage.md','scripts/runtime-tracks.mjs','scripts/motion-source-analysis.mjs','scripts/build-motion-kit.mjs',pub+file,pub+'/apple_design/app/dist/client/research/motion-catalog.json',...runtime.resources.filter(resource=>resource.file).map(resource=>pub+resource.file),...catalogFiles.map(file=>pub+file)];
const result=spawnSync('tar',['-czf',pub+'/apple_design/app/dist/client/motion-code-kit.tar.gz','--null','-T','-'],{input:[...new Set(files)].join('\0')+'\0',encoding:'utf8'});
if(result.status!==0)throw new Error(result.stderr||'Motion kit archive failed');
console.log(JSON.stringify({records:records.length,tracks:records.reduce((sum,record)=>sum+record.tracks.length,0),files:new Set(files).size,bytes:(await fs.stat(pub+'/apple_design/app/dist/client/motion-code-kit.tar.gz')).size}));
