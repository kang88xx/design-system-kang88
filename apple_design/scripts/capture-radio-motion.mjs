  ...
  for(const [viewport,width,height] of [['desktop',1440,1000],['mobile',390,844]]){
   ...
   try{
    ...
    for(const input of inputs){
     ...
     const file='/research/runtime-radio/records/'+id+'.json';await fs.writeFile(path.join(pub,file),JSON.stringify(record,null,2));output.records.push({...record,file,before:record.before?{target:record.before.target,scrollY:record.before.scrollY}:undefined,after:record.after?{target:record.after.target,scrollY:record.after.scrollY}:undefined,frames:undefined});
    }
