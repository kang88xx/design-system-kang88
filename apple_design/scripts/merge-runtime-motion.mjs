...
const supplement=JSON.parse(await fs.readFile(pub+'/research/runtime-radio-motion.json','utf8'));
...
for(const record of runtime.records.filter(record=>record.file.startsWith('/research/runtime-radio/')||(record.status==='observed'&&record.tracks?.length&&!record.sourceRefs?.length))){
...
await fs.writeFile(pub+'/research/runtime-motion.json',JSON.stringify(runtime,null,2));
