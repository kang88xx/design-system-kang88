...
const result=await page.evaluate(raw=>{
 ...
 const apiScript=doc.createElement('script');apiScript.src='/local-api.js';doc.head.prepend(apiScript);
 doc.querySelectorAll('link[rel="stylesheet"]').forEach(el=>{if(el.getAttribute('href').includes('/wss/fonts?'))el.setAttribute('href','/fonts/source-fonts.css');});
 const script=doc.createElement('script');script.src='/reference-adapter.js';script.defer=true;doc.body.append(script);
...
const local=url=>{const u=new URL(url);return u.hostname==='www.apple.com'?(u.pathname==='/wss/fonts'?'/fonts/source-fonts.css':u.pathname):'/external/'+u.hostname+u.pathname;};
async function download(url){
 ...
 try{
  ...
  const type=response?.headers.get('content-type')||(url.includes('/wss/fonts?')?'text/css':'');
...
await fs.writeFile(root+'/homepage.html',html);
await fs.writeFile(root+'/extraction.json',JSON.stringify({source:base,capturedAt:new Date().toISOString(),...Object.fromEntries(['nav','tiles','computed','icons'].map(k=>[k,result[k]])),resources:[...map].map(([url,file])=>({url,file})),failures},null,2));
